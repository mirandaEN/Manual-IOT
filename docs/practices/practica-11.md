# Práctica 11 - Sistema de secuencias múltiples con control digital y PWM por ciclos 

**Nivel:** Medio  
**Duración:** 20 minutos

## Objetivo
Programar el Particle Photon 2 para ejecutar diferentes patrones de iluminación y secuencias de encendido y apagado organizados en ciclos for, combinando control digital y modulación PWM, para dominar por completo los ciclos for
...
<div style="max-width: 700px; margin: 30px auto; padding: 25px; background: #0f172a; border-radius: 20px; text-align: center; color: white; position: relative;">
  ...
  <div style="position: relative; display: inline-block;">
    <img src="/manual-iot/assets/images/practica2-diagrama.jpeg" 
         alt="Circuito 4 LEDs - Práctica 11" 
  ...
  <div style="margin-top: 25px;">
    <button onclick="toggleSimulation11()" 
            id="btnSim"
...
function runNextCycle(leds, status) {
  ...
  const cycleNames = [
    "1. Llenado y vaciado progresivo",
    "2. PWM simultáneo en MISO y A2 (breathing)",
    "3. Parpadeo individual ida y vuelta"
  ];
  ...
  interval = setInterval(() => {
    ...
    if (currentCycle === 0) {
      // Ciclo 1: Llenado progresivo → vaciado progresivo
      if (i < 4) {
        for (let k = 0; k <= i; k++) {
          leds[k].style.boxShadow = '0 0 45px 30px rgba(255, 80, 80, 0.95)';
          leds[k].style.background = 'radial-gradient(circle at 30% 30%, #ff6666, #ff2222)';
        }
      } else {
        const off = i - 4;
        for (let k = 0; k < 4 - off; k++) {
          leds[k].style.boxShadow = '0 0 45px 30px rgba(255, 80, 80, 0.95)';
          leds[k].style.background = 'radial-gradient(circle at 30% 30%, #ff6666, #ff2222)';
        }
      }
      ...
      // Ciclo 2: PWM breathing simultáneo en LED1 (MISO) y LED3 (A2)
      const brightness = Math.sin(Date.now() / 300) * 0.5 + 0.5;
      const intensity = Math.floor(brightness * 255);
      leds[0].style.boxShadow = `0 0 ${25 + intensity * 0.15}px ${12 + intensity * 0.1}px rgba(255, 100, 100, 0.95)`;
      leds[0].style.background = 'radial-gradient(circle at 30% 30%, #ff8888, #ff3333)';
      leds[2].style.boxShadow = `0 0 ${25 + intensity * 0.15}px ${12 + intensity * 0.1}px rgba(255, 100, 100, 0.95)`;
      leds[2].style.background = 'radial-gradient(circle at 30% 30%, #ff8888, #ff3333)';
    } 
    else {
      // Ciclo 3: Parpadeo individual ida y vuelta
      leds[i % 4].style.boxShadow = '0 0 45px 30px rgba(255, 80, 80, 0.95)';
      leds[i % 4].style.background = 'radial-gradient(circle at 30% 30%, #ff6666, #ff2222)';
      i++;
    }
...
# void loop() {
    //PRIMER CICLO
    //ENCERNDER SIN APAGAR TODOS LOS PINES
    if (primerCiclo > 0) {
    ...
    delay(300);  // pausa más larga al terminar la repetición completa
...
## Resultado Esperado
Se espera que los leds ejecuten con éxito los 3 ciclos que se indicaron.

## Evidencia
![Practica 11](../assets/images/practica11.jpeg)

![Captura Web IDE](../assets/images/practica11-captura.jpeg)

## Ver Video
<video width="50%" controls>
    <source src="/manual-iot/assets/videos/practica11.mp4" type="video/mp4">
