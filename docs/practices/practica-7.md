# Práctica 7 - Secuencias de LEDs con diferentes tiempos de encendido y apagado

**Nivel:** Fácil  
**Duración:** 15 minutos

## Objetivo
Programar el Particle Photon 2 para generar distintos efectos de iluminación en 5 LEDs utilizando arreglos, ciclos for y diferentes tiempos de encendido y apagado.
...
<div style="max-width: 700px; margin: 30px auto; padding: 25px; background: #0f172a; border-radius: 20px; text-align: center; color: white; position: relative;">
  ...
  <div style="position: relative; display: inline-block;">
    <img src="/manual-iot/assets/images/practica4-diagrama.jpeg" 
         alt="Circuito 5 LEDs - Práctica 7" 
  ...
  <div style="margin-top: 25px;">
    <button onclick="toggleSimulation7()" 
            id="btnSim"
  ...
  <p id="statusSim" style="margin-top: 12px; font-size: 15px; color: #64748b;">
    Secuencia 1 → Secuencia 2 → Secuencia 3 (se repite)
  </p>
</div>
...
function toggleSimulation7() {
  ...
  if (running) {
    ...
    running = true;
    currentSequence = 0;
    runNextSequence(leds, status);
  }
}

function runNextSequence(leds, status) {
  if (!running) return;

  const sequences = [
    { name: "1. Ida y vuelta rápida (250ms)", delay: 250 },
    { name: "2. Encendido progresivo + apagado inverso", delay: 125 },
    { name: "3. Parpadeo lento (500ms)", delay: 500 }
  ];

  status.innerHTML = `✅ <strong>${sequences[currentSequence].name}</strong>`;
  ...
  interval = setInterval(() => {
    ...
    if (currentSequence === 0) { 
      // Secuencia 1: ida y vuelta rápida (individual)
      ...
      // Secuencia 2: llenado progresivo + vaciado inverso
      if (i < 5) {
        ...
        const off = i - 5;
        for (let k = 0; k < 5 - off; k++) leds[k].style.boxShadow = '0 0 45px 30px rgba(255, 80, 80, 0.95)';
        for (let k = 0; k < 5 - off; k++) leds[k].style.background = 'radial-gradient(circle at 30% 30%, #ff6666, #ff2222)';
      }
      i++;
      if (i >= 10) i = 0;
    } 
    else { 
      // Secuencia 3: parpadeo lento ida y vuelta
      leds[i].style.boxShadow = '0 0 45px 30px rgba(255, 80, 80, 0.95)';
      leds[i].style.background = 'radial-gradient(circle at 30% 30%, #ff6666, #ff2222)';
      i += direction;
      if (i >= 4) direction = -1;
  ...
  }, sequences[currentSequence].delay);

  // Cambiar de secuencia cada 4-5 segundos
  setTimeout(() => {
    if (running) {
      clearInterval(interval);
...
## Resultado Esperado
Se espera que los LEDs ejecuten tres patrones diferentes de iluminación de manera continua.

## Evidencia
![Practica 7](../assets/images/practica7.jpeg)

![Captura Web IDE](../assets/images/practica7-captura.jpeg)

## Ver Video
<video width="50%" controls>
  <source src="/manual-iot/assets/videos/practica7.mp4" type="video/mp4">
